/**
 * https://github.com/kothariji/use-media-stream/blob/master/src/index.ts
 */
...
const useMediaStream = (props?: useMediaStreamInterface) => {
  ...
  const [mediaDeviceConstraints, setMediaDeviceConstraints] = useState(() =>
    deepmerge(defaultMediaDeviceConstraints, props?.mediaDeviceConstraints ?? {})
  );
  ...
  devices.forEach((d: MediaDeviceInfo) => {
    ...
    } else if ((d.kind as string) === 'videoinput') {
  ...
  const initiateStream = async (
    mediaDeviceConstraintsFromArgs: MediaStreamConstraints = mediaDeviceConstraints
  ...
  const updateMediaDeviceConstraints = async ({
    ...
    const updatedUserMediaConstraints = deepmerge(mediaDeviceConstraints, constraints);
